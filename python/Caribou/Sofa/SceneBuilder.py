...
class SceneBuilder(object):
    ...
    def get_mesh_object(self, mesh):
        try:
            ...
            if not isinstance(mesh, CGAL.VolumetricMesh):
                ...
                mesh.create(self.simulation, self.meshes_node)
    ...
    def add_linear_solver(self, node, solver):
        ...
        elif isinstance(solver, LUSolver):
            node.createObject(
                'SparseLUSolver',
                tolerance=solver.tolerance,
                verbose=solver.verbose,
    ...
    def add_pde_solver(self, node):
        ...
        if isinstance(pde_solver, NewtonRaphsonSolver):
            node.createObject(
                'NewtonRaphsonSolver',
                maxIt=pde_solver.maxIt,
                correctionTolerance=pde_solver.correctionTolerance,
                residualTolerance=pde_solver.residualTolerance,
                convergeOnResidual=pde_solver.convergeOnResidual,
                printLog=pde_solver.printLog,
                cutoff=pde_solver.cutoff,
            )

        elif isinstance(pde_solver, StaticSolver):
            node.createObject(
                'StaticSolver',
                printLog=pde_solver.printLog,
            )
        ...
        if isinstance(pde_solver, (StaticSolver, ImplicitEuler)):
            self.add_linear_solver(node, pde_solver.solver)
        ...
        for boundary in boundaries:
            if isinstance(boundary, FixedBoundary):
                ...
                node.createObject(
                    ...
                    number_of_steps_before_increment=boundary.number_of_steps_before_increment,
