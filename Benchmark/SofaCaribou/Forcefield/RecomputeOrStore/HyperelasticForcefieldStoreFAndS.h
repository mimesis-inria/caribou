...
class HyperelasticForcefieldStoreFAndS : public ::SofaCaribou::forcefield::CaribouForcefield<Element> {
public:
    SOFA_CLASS(SOFA_TEMPLATE(HyperelasticForcefieldStoreFAndS, Element), SOFA_TEMPLATE(::SofaCaribou::forcefield::CaribouForcefield, Element));
    ...
    using Link = sofa::core::objectmodel::SingleLink<HyperelasticForcefieldStoreFAndS<Element>, ObjectType, sofa::core::objectmodel::BaseLink::FLAG_STRONGLINK>;

    // Data structures
    struct GaussNode {
        Real weight;
        Real jacobian_determinant;
        Real deformation_determinant;
        Matrix<NumberOfNodesPerElement, Dimension> dN_dx;
        Mat33 F = Mat33::Identity(); // Deformation gradient
        Mat33 C = Mat33::Identity(); // Strain tensor
        Mat33 S = Mat33::Identity(); // Stress tensor
    };
