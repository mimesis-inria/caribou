...
TEST(BarycentricContainer, _2D) {
    ...
    for (const auto & p : positions) {
        ...
        EXPECT_GT(res.element_index, -1);
        if (res.element_index > -1) {
            const Quad element = container_domain->element(res.element_index);
            EXPECT_MATRIX_NEAR(p, element.world_coordinates(res.local_coordinates).transpose(), 1e-10);
        }
    }
    ...
    for (UNSIGNED_INTEGER_TYPE node_id = 0; node_id < (unsigned) interpolated_values.rows(); ++node_id) {
        EXPECT_MATRIX_EQUAL(interpolated_values.row(node_id), embedded_positions_1[node_id]);
    }
