...
void create_segment(pybind11::module & m) {
    ...
    m.def("Segment", [](const caribou::python::Dimension & dim) {
    ...
    m.def("Segment", [](const caribou::python::Order & order) {
    ...
    m.def("Segment", [](const caribou::python::Dimension & dim, const caribou::python::Order & order) {
        if (dim == caribou::python::Dimension::_1D) {
            if (order == caribou::python::Order::Linear)
                return py::cast(Segment<_1D, Linear>());
            else
