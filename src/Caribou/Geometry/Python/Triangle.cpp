...
void create_triangle(pybind11::module & m) {
    ...
    m.def("Triangle", [](const caribou::python::Dimension & dim) {
    ...
    m.def("Triangle", [](const caribou::python::Order & order) {
    ...
    m.def("Triangle", [](const caribou::python::Dimension & dim, const caribou::python::Order & order) {
        if (dim == caribou::python::Dimension::_2D) {
            if (order == caribou::python::Order::Linear)
                return py::cast(Triangle<_2D, Linear>());
            else
    ...
    // Linear creation
    // 2D
    m.def("Triangle", [](const Eigen::Matrix<FLOATING_POINT_TYPE, 3, 2> & nodes, const caribou::python::Order & order) {
    ...
    }, py::arg("n0"), py::arg("n1"), py::arg("n2"), py::arg("order") = caribou::python::Order::Linear);

    // 3D
    m.def("Triangle", [](const Eigen::Matrix<FLOATING_POINT_TYPE, 3, 3> & nodes, const caribou::python::Order & order) {
        return (order == caribou::python::Order::Linear)
