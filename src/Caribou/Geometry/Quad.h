...
struct Quad <_Dimension, Linear> : public BaseQuad<Quad <_Dimension, Linear>> {
    ...
    Quad() : Base() {
        if constexpr (Dimension == 2) {
            this->p_nodes.row(0) = WorldCoordinates(-1, -1); // Node 0
            this->p_nodes.row(1) = WorldCoordinates(+1, -1); // Node 1
            this->p_nodes.row(2) = WorldCoordinates(+1, +1); // Node 2
            this->p_nodes.row(3) = WorldCoordinates(-1, +1); // Node 3
        } else {
            this->p_nodes.row(0) = WorldCoordinates(-1, -1, 0); // Node 0
            this->p_nodes.row(1) = WorldCoordinates(+1, -1, 0); // Node 1
            this->p_nodes.row(2) = WorldCoordinates(+1, +1, 0); // Node 2
            this->p_nodes.row(3) = WorldCoordinates(-1, +1, 0); // Node 3
        }
    };
...
struct Quad <_Dimension, Quadratic> : public BaseQuad<Quad <_Dimension, Quadratic>> {
    ...
    Quad() : Base() {
        if constexpr (Dimension == 2) {
            ...
            this->p_nodes.row(2) = WorldCoordinates(+1, +1); // Node 2
            this->p_nodes.row(3) = WorldCoordinates(-1, +1); // Node 3
            this->p_nodes.row(4) = WorldCoordinates( 0, -1); // Node 4
            this->p_nodes.row(5) = WorldCoordinates(+1,  0); // Node 5
            this->p_nodes.row(6) = WorldCoordinates( 0, +1); // Node 6
            this->p_nodes.row(7) = WorldCoordinates(-1,  0); // Node 7
        } else {
            this->p_nodes.row(0) = WorldCoordinates(-1, -1, 0); // Node 0
            this->p_nodes.row(1) = WorldCoordinates(+1, -1, 0); // Node 1
            this->p_nodes.row(2) = WorldCoordinates(+1, +1, 0); // Node 2
            this->p_nodes.row(3) = WorldCoordinates(-1, +1, 0); // Node 3
            this->p_nodes.row(4) = WorldCoordinates( 0, -1, 0); // Node 4
            this->p_nodes.row(5) = WorldCoordinates(+1,  0, 0); // Node 5
            this->p_nodes.row(6) = WorldCoordinates( 0, +1, 0); // Node 6
            this->p_nodes.row(7) = WorldCoordinates(-1,  0, 0); // Node 7
        }
    };

    // Construct a quadratic Quad from a linear one
    Quad(const Quad<Dimension, Linear> & linear_Quad) : Base() {
        ...
        this->p_nodes.row(4) = linear_Quad.T(LocalCoordinates( 0, -1)); // Node 4
        this->p_nodes.row(5) = linear_Quad.T(LocalCoordinates(+1,  0)); // Node 5
        this->p_nodes.row(6) = linear_Quad.T(LocalCoordinates( 0, +1)); // Node 6
        this->p_nodes.row(7) = linear_Quad.T(LocalCoordinates(-1,  0)); // Node 7
    }
