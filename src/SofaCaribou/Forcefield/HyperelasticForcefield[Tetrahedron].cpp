...
template <>
void HyperelasticForcefield<Tetrahedron<Linear>>::create_domain_from(BaseMeshTopology * topology) {
...
template <>
auto HyperelasticForcefield<Tetrahedron<Linear>>::mesh_is_compatible(const BaseMeshTopology * topology) -> bool {
    return (
...
// -----------------------------------
template <>
void HyperelasticForcefield<Tetrahedron<Quadratic>>::create_domain_from(BaseMeshTopology *) {
    // We cannot create a quadratic topology from a linear one
}

template <>
auto HyperelasticForcefield<Tetrahedron<Quadratic>>::mesh_is_compatible(const BaseMeshTopology *) -> bool {
    // We cannot create a quadratic topology from a linear one
    return false;
}

template <>
auto HyperelasticForcefield<Tetrahedron<Quadratic>>::templateName(const HyperelasticForcefield<Tetrahedron<Quadratic>> *) -> std::string {
    return "Tetrahedron10";
}

template <>
void HyperelasticForcefield<Tetrahedron<Quadratic>>::draw(const sofa::core::visual::VisualParams * vparams) {
    using Color = sofa::helper::types::RGBAColor;
    constexpr static auto NumberOfFaces = 4;
    ...
    for (std::size_t face_id = 0; face_id < NumberOfFaces; ++face_id) {
        faces_points[face_id].reserve(nb_elements*3);
    }
    ...
    for (std::size_t element_id = 0; element_id < nb_elements; ++element_id) {
        ...
        const auto e = Tetrahedron<Quadratic>(element_nodes_position);
        ...
        for (std::size_t face_id = 0; face_id < NumberOfFaces; ++face_id) {
            for (std::size_t face_node_id = 0; face_node_id < 3; ++face_node_id) {
