...
void FictitiousGrid<DataTypes>::init() {
    ...
    if (not d_surface_topology_container.get()) {
        ...
        if (containers.empty() || containers.size() > 1) {
            ...
            for (TopologyContainer * c : containers) {
                ...
                else if(Dimension == 3 and ( c->getTopologyType() == sofa::core::topology::TRIANGLE ||  c->getTopologyType() == sofa::core::topology::QUAD))
                    surface_containers.emplace_back(c);
            }
            ...
            if (surface_containers.empty()) {
                msg_error()
                        ...
                        << "Please set the parameter '" << d_surface_topology_container.getName() << "' to the path of the topology container that "
                ...
                msg_info() << "Automatically found the topology container at '" << surface_containers[0]->getPathName() << "'.";
            }
        } else {
    ...
    if (Dimension == 2 and topology->getNbEdges() == 0) {
        msg_warning() << "The topology container '" << topology->getPathName() << "' does not contain any edge elements.";
    } else if (Dimension == 3 and (topology->getNbTriangles() == 0 and topology->getNbQuads() == 0)) {
        msg_warning() << "The topology container '" << topology->getPathName() << "' does not contain any triangle or quad elements.";
    }
        ...
        if (x.empty()) {
            ...
            if (d_surface_positions.getParent()) {
                ...
                if (mechanical_states.empty()) {
                    ...
                    const auto rest_positions_data = dynamic_cast<Data<SofaVecCoord>*> (mo->findData("rest_position"));
                    if (!rest_positions_data) {
                        ...
                        if (rest_positions.empty()) {
                            ...
                            msg_info() << "Automatically found the positions vector at '" << mo->getPathName() + "." + rest_positions_data->getName() << "'.";
                        }
                    }
                }
            }
