...
void CaribouBarycentricMapping<Element, MappedDataTypes>::apply(const sofa::core::MechanicalParams * /*mparams*/,
    ...
    if constexpr (is_rigid_types_v<MappedDataTypes>) {
        for (std::size_t bp_index = 0; bp_index <nb_of_barycentric_points; ++bp_index) {
            ...
            mapped_positions(bp_index, Dimension+0) = q0.w(); //(q*q0).w();
            mapped_positions(bp_index, Dimension+1) = q0.x(); //(q*q0).x();
            mapped_positions(bp_index, Dimension+2) = q0.y(); //(q*q0).y();
            mapped_positions(bp_index, Dimension+3) = q0.z(); //(q*q0).z();
        }
    } else {
